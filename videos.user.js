...
(() => {
  ...
  function showSpeedMenu(anchor) { const m = createMenu(rates.map(r => r + 'x'), t => { currVid.playbackRate = parseFloat(t); updateRateDisplay(parseFloat(t)); }); placeMenu(m, anchor); }
  function showSizeMenu(anchor) { const m = createMenu(Object.keys(sizes), t => { currVid.style.objectFit = sizes[t]; updateSizeDisplay(t); }); placeMenu(m, anchor); }
